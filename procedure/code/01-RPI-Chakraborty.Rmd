...
In the original analysis, Chakraborty used socio-demographic and poverty data from ACS five-year estimates for 2018 and the Johns Hopkins county-level COVID-19 incidence data downloaded on August 1, 2020. With these data, he created map showing the distribution of COVID-19 incidence rate, calculated the descriptive statistics, and measured the bivariate relationship between between COVID-19 incidence and disability subgroups taking into account covid clusters. 

**each of the bivariate correlations is also testing a hypothesis**
...
**Discuss only the data for the replication. To what extent have you already accessed/viewed/analyzed the data in preparation of this preanalysis plan?** This registration is based upon a thorough reading of the original research article, understanding the research method, reproducing the original research, and acquiring some additional information and data from the original author, Jay Chakraborty. We plan to replace it with the latest ACS 2020 data currently available on the US Census Bureau. Chakraborty informed us of the American Community Survey data table names used in the original study (S1810 for demographic categories and disability status and C18130 for poverty status and disability status) and those table names remain the same in 2020. In our replication attempt, we will use publicly available ACS data downloaded directly from the Census API using the tidycensus package for R. In our replication analysis, we attempt to replace COVID-19 incidence data with the latest COVID-19 death. This is because Chakraborty conducted the analysis at the early stage of the pandemic when COVID-19 incidence rate most evidently reflects the severity of the pandemic in each county. However, COVID-19 incidence rate is no longer the most ideal indicator of the differences of county level pandemic severity at the time of our replication analysis since the virus has become more contagious and self antigen test has become a more popular means to detect the virus. We plan to obtain the data at he John Hopkins CCSE COVID-19 Data Repository (https://github.com/CSSEGISandData/COVID-19), which will express cumulative count of reported COVID-19 from 1/22/2020 to 7/2/2022. We also re-conceptualize the cluster based on state and whether the county is urban or rural. We attempt to use the data provided by USDA that places counties along the urban-rural continuum, available for download at https://ers.usda.gov/data-products/rural-urban-continuum-codes/
...
The socio-demographic/poverty data and the death rate data are grouped by U.S. Census county cluster first. County clusters group contiguous rural counties together until they form an aggregate population of at least **min population**. Based on the cluster, we calculated the first order queen contiguity, the spatial weight for neighbor's list, and the G statistics before mapping the hotspots and coldspots.
...
The study will **weight** county and county clusters by total population.
