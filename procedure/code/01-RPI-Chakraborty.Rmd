...
The **spatial extent** of the original study are the 49 contiguous states in the U.S.
...
### Calculate weighted Spearman Rho's Correlation Coefficient
```{r}
...
temp <- temp %>%  
  filter(!is.na(excess_death_rate_2020)) %>%
  arrange()
weightedCorr(y = wcorr_data$excess_death_rate_2020, x = wcorr_data$dis_pct, weights = wcorr_data$total_pop, method = "Spearman")
...
res <- purrr::map_dfr(to_do_list,
               ~tibble(var_row = .[[1]],
                       ...
                       wCorr = wCorr::weightedCorr(x = temp[[.[[1]]]],
                                                   ...
                                                   method = "Spearman"),
                       Corr = wCorr::weightedCorr(x = temp[[.[[1]]]],
                                                  y= temp[[.[[2]]]],
                                                  method = "Spearman"))) 
